...
class NewUserTable extends TableAbstract
{
    ...
    public function __construct(
        ...
        $this->setOption('id', 'table-packages-hr-new-user');
        parent::__construct($table, $urlDevTool);

        if (!Auth::user()->hasAnyPermission(['users.edit', 'users.destroy', 'hr.activate'])) {
    ...
    public function query()
    {
        ...
        $query = $model
            ...
            ->whereNotIn('users.id', function ($query) {
                ...
                $query->select('activations.user_id')->from('activations');
            });
            ...
            if (Auth::user()->getPosition()->first()->code == 'GDCN') {
                $query = $model
                    ->whereHas('positions', function ($query) {
                        return $query->where('branch_id', Auth::user()->getBranch()->first() ? Auth::user()->getBranch()->first()->id : null)
                            ->where('position_id', '!=', Auth::user()->getPosition()->first() ? Auth::user()->getPosition()->first()->id : null);
                    });
            }
        }
