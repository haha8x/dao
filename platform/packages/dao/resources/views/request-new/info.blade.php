<div class="modal-box-container">
    <div class="modal-title">
        <i class="til_img"></i> <strong>{{ __('Xem yêu cầu mã DAO:id', ['id' => $daoRequestNew->id]) }}</strong>
    </div>
    ...
    <div class="modal-footer">
        ...
        <a href="{{ route('dao-request-new.receive', $daoRequestNew->id) }}" class="btn btn-info">
            {{ __('Tiếp nhận') }}
        </a>
        @endif
        @if ($daoRequestNew->status == 'receive')
        <a href="{{ route('dao-request-new.reject', $daoRequestNew->id) }}" class="btn btn-info">
            {{ __('Từ chối') }}
        </a>
        @endif
        @if ($daoRequestNew->status == 'receive')
        <a href="{{ route('dao-request-new.approve', $daoRequestNew->id) }}" class="btn btn-info">
            {{ __('Duyệt') }}
        </a>
        @endif
        @if ($daoRequestNew->status == 'gdcn_approve')
        <a href="{{ route('dao-request-new.it_process', $daoRequestNew->id) }}" class="btn btn-info">
            {{ __('IT Xử lý') }}
        </a>
        @endif
