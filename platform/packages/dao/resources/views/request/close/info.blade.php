<div class="modal-box-container">
    ...
    <div class="modal-body">
        <div class="request-action" style="float: right;margin-bottom: 10px;">
            ...
            @if (Auth::user()->hasPermission('request-close.tiep_nhan'))
            <a href="{{ route('request-close.tiep_nhan', $item->id) }}" class="btn btn-info">
                {{ __('Tiếp nhận') }}
            </a>
            @endif

            @if (Auth::user()->hasPermission('request-close.tu_choi'))
            <a href="{{ route('request-close.tu_choi', $item->id) }}" class="btn btn-danger">
                {{ __('Từ chối') }}
            </a>
            @endif
            @endif
            ...
            @if (Auth::user()->hasPermission('request-close.gdcn_duyet'))
            <a href="{{ route('request-close.gdcn_duyet', $item->id) }}" class="btn btn-info">
                {{ __('GDCN Duyệt') }}
            </a>
            @endif
            @if (Auth::user()->hasPermission('request-close.hoiso_duyet'))
            <a href="{{ route('request-close.hoiso_duyet', $item->id) }}" class="btn btn-info">
                {{ __('Hội sở Duyệt') }}
            </a>
            @endif
            @endif
            ...
            @if (Auth::user()->hasPermission('request-close.it_xuly'))
            <a href="{{ route('request-close.it_xuly', $item->id) }}" class="btn btn-info">
                {{ __('IT Xử lý') }}
            </a>
            @endif
            @endif
            ...
            @if (Auth::user()->hasPermission('request-close.thanh_cong'))
            <a href="{{ route('request-close.thanh_cong', $item->id) }}" class="btn btn-info">
                {{ __('Thành công') }}
            </a>
            @endif
            @endif
